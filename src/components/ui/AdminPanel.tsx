...
export default function AdminPanel({ currentRole }: AdminPanelProps) {
  ...
  const handlePublishAll = () => {
    // 1. Verificar si existen archivos con error estructural
    const hasErrors = catalogs.some(c => c.status === 'Con error');
    if (hasErrors) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Validation Alert Modal (Safeguard Control 1) */}
      {showValidationAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-slate-300 leading-relaxed">
                No es posible realizar la publicación de catálogos. Se han detectado **archivos con errores estructurales pendientes** en el módulo de control de calidad.
              </p>
              <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl p-3.5 flex gap-3 items-start">
                <ShieldAlert className="w-5 h-5 text-rose-450 shrink-0 mt-0.5" />
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Para garantizar la precisión de las respuestas del chatbot y evitar cotizaciones erróneas, debes depurar o eliminar los archivos en estado de error antes de continuar.
                </p>
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-slate-950/40 border-t border-slate-800 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowValidationAlert(false)}
                className="bg-slate-850 hover:bg-slate-800 text-slate-400 hover:text-slate-200 px-4 py-2 rounded-xl text-xs font-semibold transition-all active:scale-95"
              >
                Cancelar
              </button>
              <button
                ...
                onClick={() => {
                  ...
                  const firstErrorCatalog = catalogs.find(c => c.status === 'Con error');
                  if (firstErrorCatalog) {
                    setSelectedCatalogForRepair(firstErrorCatalog);
                  }
                }}
                className="px-4 py-2 text-xs font-bold text-white bg-amber-600 hover:bg-amber-500 rounded-xl shadow-md transition-all active:scale-95 flex items-center gap-1.5"
              >
                <RefreshCw className="w-3.5 h-3.5" />
                Revisar errores
              </button>
