  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-stretch sm:items-center p-4 bg-slate-950/60 border border-slate-900/80 rounded-2xl backdrop-blur-xl relative z-20">
        ...
        {/* Dropdown status Filter and Add button */}
        <div className="flex items-center gap-3 justify-end shrink-0">
          ...
          <div className="relative" ref={categoryDropdownRef}>
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-slate-900/60 hover:bg-slate-850 border border-slate-850 hover:border-slate-800 rounded-xl text-xs font-semibold text-slate-300 hover:text-slate-100 transition-all active:scale-[0.98]"
            >
              <span>Categoría: <strong className="capitalize text-slate-200">{categoryFilter}</strong></span>
              <ChevronDown className="w-3.5 h-3.5 text-slate-500" />
            </button>

            {isCategoryDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 rounded-xl bg-slate-900 border border-slate-800 shadow-xl overflow-hidden z-30 py-1 animate-in fade-in slide-in-from-top-2 duration-150">
                {categoriesList.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-xs font-semibold transition-all capitalize ${
                      categoryFilter === cat 
          ...
          {canModify && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-bold transition-all active:scale-[0.98] shadow-md shadow-cyan-950/20"
            >
